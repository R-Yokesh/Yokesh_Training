<?php 
  ...
  if(isset($_GET)){
    foreach($_GET as $key=>$val){
      $u = mysqli_real_escape_string($conn, $key); // Sanitize the URL key
      $new_url = str_replace('/', '', $u); // Remove slashes from the key
    }
    $sql = mysqli_query($conn, "SELECT full_url FROM url WHERE shorten_url = '{$new_url}'"); // Check if the short URL exists
    if(mysqli_num_rows($sql) > 0){
      $sql2 = mysqli_query($conn, "UPDATE url SET clicks = clicks + 1 WHERE shorten_url = '{$new_url}'"); // Update click count
      if($sql2){
...
<body>
  <div class="wrapper">
    ...
    <?php
      $sql2 = mysqli_query($conn, "SELECT * FROM url ORDER BY id DESC"); // Fetch all URLs
      if(mysqli_num_rows($sql2) > 0){
        ?>
        <div class="statistics">
          <?php
            $sql3 = mysqli_query($conn, "SELECT COUNT(*) FROM url"); // Count total links
            $res = mysqli_fetch_assoc($sql3);

            $sql4 = mysqli_query($conn, "SELECT clicks FROM url"); // Count total clicks
            $total = 0;
        ...
        <div class="urls-area">
          ...
          <?php
            while($row = mysqli_fetch_assoc($sql2)){ // Display each URL's details
              ?>
              <div class="data">
                <li>
                  ...
                  <?php
                    if($domain.strlen($row['shorten_url']) > 50){
                      echo $domain.substr($row['shorten_url'], 0, 50) . '...'; // Shorten display if too long
                    }else{
                ...
                <li>
                  <?php
                    if(strlen($row['full_url']) > 60){
                      echo substr($row['full_url'], 0, 60) . '...'; // Shorten display if too long
                    }else{
                      echo $row['full_url'];
                    }
                  ?>
